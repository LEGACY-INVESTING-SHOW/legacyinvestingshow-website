...
Current published LIS offers from `llms.txt` and `/reviews` (confirm live before reuse): Legacy Wealth Blueprint $9,800 cash or card ($10,800 Splitit); LWB Course + AI $1,500; Airbnb Ascension $9,800; Airbnb Ascension Scale $18,000; STR Concierge $16,000; STR Concierge Portfolio $30,000; bundles $17,000 to $34,000. Free entry: https://join.managemoney101.com/tax-strategies.

Older success-story copy still quotes "$2,497" as a student line about community value. That is not the current list price. Do not ship $2,497 as what Blueprint costs.

Portfolio claims: prefer the current homepage/about figure. Do not invent $15M vs $20M.
...
| 17 | `splitit-vs-cash-for-coaching` | pay in installments vs cash for a course | decision | Financing a class is still debt | Our Splitit figure; FTC rush warning | LWB cost page | Low |
...
1. Write `/compare/{slug}` as JSON. Type `{alternatives|vs|best-for|review|checklist|decision}`. Grade 4-5 English. No em dashes. First `answer` field must answer the query in under 120 words. Disclose that Legacy Investing Show sells Legacy Wealth Blueprint at the current published cash price. Cite every competitor fact with url + asOf.

2. Alternatives page for `{brand}`. Group by job, not by lookalike branding. Include a free option, a cheap published-price option, a licensed pro, and LWB marked as ours. Skip software lead tools unless the query is software.

3. Vs page for `{A}` vs `{B}`. Define both terms on first use. One table: topic / A / B / plain read. No winner trophy. WhoFits and WhoSkips required.
