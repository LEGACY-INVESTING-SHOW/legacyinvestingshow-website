...
const { renderSiteHeader, renderSiteFooter, renderHeadAssets } = require('./lib/site-shell');
const { renderSkipLink, renderToolsSubnav } = require('./lib/tools-shell');
...
try {
    ...
    const output = template
        .replace('{{TAX_TABLES}}', JSON.stringify(tables).replace(/</g, '\\u003c'))
        .replace('{{HEAD_ASSETS}}', renderHeadAssets())
        .replace('{{SITE_SKIP}}', renderSkipLink())
        .replace('{{SITE_HEADER}}', renderSiteHeader('/tools'))
        .replace('{{TOOLS_SUBNAV}}', renderToolsSubnav('/tools'))
        .replace('{{SITE_FOOTER}}', renderSiteFooter());
    if (output.includes('{{')) throw new Error('Unreplaced placeholder in tax-structure-calculator.html');
