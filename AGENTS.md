...
## Unlisted Course Libraries

Paid client libraries live at obscure, unlisted URLs. Do not add them to nav, footer, sitemap, `llms.txt`, `llms-full.txt`, or internal links.

- Legacy Wealth Blueprint: `/lx/wbp-26`
- Airbnb Arbitrage Roadmap: `/lx/str-rd`

These pages are `noindex` and `robots.txt` Disallow. They are imported from the coursewebs knowledge bases, not generated by the blog/CMS pipeline. Do not rewrite them during site-wide SEO/tracking passes.
